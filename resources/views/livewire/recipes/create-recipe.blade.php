    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Add a new recipe to your collection</p>
            </div>
            <a href="{{ route('recipes') }}" wire:navigate.hover class="text-gray-400 hover:text-gray-600">
        ...
        <form x-on:submit.prevent="saveRecipe()" class="space-y-8">
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                <a 
                    href="{{ route('recipes') }}" wire:navigate.hover
                ...
                >
                    <span x-show="!saving">Create Recipe</span>
...
function recipeForm() {
    return {
        ...
        init() {
            // Initialize with any existing form data from Livewire
            @if($form->title)
                this.form.title = @json($form->title);
            @endif
            @if($form->description)
                this.form.description = @json($form->description);
            @endif
            @if($form->category)
                this.form.category = @json($form->category);
            @endif
            @if($form->difficulty)
                this.form.difficulty = @json($form->difficulty);
            @endif
            @if($form->prep_time)
                this.form.prep_time = @json($form->prep_time);
            @endif
            @if($form->cook_time)
                this.form.cook_time = @json($form->cook_time);
            @endif
            @if($form->servings)
                this.form.servings = @json($form->servings);
            @endif
            @if($form->is_public)
                this.form.is_public = @json($form->is_public);
            @endif
            @if($form->ingredients)
                this.form.ingredients = @json($form->ingredients);
            @endif
            @if($form->instructions)
                this.form.instructions = @json($form->instructions);
            @endif
            @if($form->tags)
                this.form.tags = @json($form->tags);
            @endif
        },
